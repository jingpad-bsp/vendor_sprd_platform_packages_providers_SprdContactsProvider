...
public class DataRowHandlerForGroupMembership extends DataRowHandler {
    ...
    public long insert(SQLiteDatabase db, TransactionContext txContext, long rawContactId,
        ...
        if (simProxyUtil.isMyContact(rawContactId)) {
            //UNISOC: add for bug1157049, buildSimContactGroupCache after acore lowmemorykiller
            Long groupRowId = values.getAsLong(GroupMembership.GROUP_ROW_ID);
            if (simProxyUtil.isMyContactGroup(groupRowId)) {
                contactProxyManager.onDataUpdate(rawContactId, values,
    ...
    public int delete(SQLiteDatabase db, TransactionContext txContext, Cursor c) {
         ...
         * @{
         */
        String origGroupRowId = c.getString(DataDeleteQuery.DATA1);
        ContentValues tmp = new ContentValues();
        tmp.put(GroupMembership.GROUP_ROW_ID, (Long) null);
        tmp.put(GroupMembership.GROUP_ROW_ID + "_orig", origGroupRowId);
        ContactProxyManager contactProxyManager = ContactProxyManager.getInstance(mContext);
        SimContactProxyUtil simProxyUtil = SimContactProxyUtil.getInstance(contactProxyManager, mContext);
        if (simProxyUtil.isMyContact(rawContactId)) {
            contactProxyManager.onDataUpdate(rawContactId, tmp, GroupMembership.CONTENT_ITEM_TYPE);
        }
        /**
         * @}
         */
