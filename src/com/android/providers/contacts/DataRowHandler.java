...
public abstract class DataRowHandler {
    ...
    public interface DataUpdateQuery {
         ...
         * @{
         */
        String[] COLUMNS = { Data._ID, Data.RAW_CONTACT_ID, Data.MIMETYPE, Data.DATA1, Data.DATA2 };
        /**
         * @}
         */
    ...
    public void appendSearchableData(SearchIndexManager.IndexBuilder builder) {
         ...
         * @{
         */
        String value = builder.getString(Data.DATA1);
    ...
    protected void fixRawContactDisplayName(SQLiteDatabase db, TransactionContext txContext,
        ...
        if (!isNewRawContact(txContext, rawContactId)) {
             ...
             * @{
             */
            mDbHelper.updateRawContactIdentity(db, rawContactId);
            /**
             * @}
             */
    ...
    public static void appendSearchableData(SearchIndexManager.IndexBuilder builder,
            String searchableValue) {
        if (TextUtils.isEmpty(searchableValue)) {
            return;
        }
        builder.appendToken(searchableValue);
        int len = searchableValue.length();
        for (int i = 1; i < len; ++i) {
            builder.appendToken(searchableValue.substring(i,
                    Math.min(i + MAX_SEARCH_SNIPPET_SIZE, len)));
        }
    }
    /**
     * @}
