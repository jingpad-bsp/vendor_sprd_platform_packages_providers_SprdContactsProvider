...
public class ContactsDatabaseHelper extends SQLiteOpenHelper {
    ...
    public interface Joins {
         ...
         */
        /**
         * SPRD BUG 811435 Remove the deleted contacts in the group@{
         **/
        public static final String GROUP_MEMBER_COUNT =
                " LEFT OUTER JOIN (SELECT "
                    ...
                    + "FROM (data join raw_contacts on data.raw_contact_id = raw_contacts._id) "
                    + "WHERE "
    ...
    public interface Clauses {
        ...
        String NOT_SYNCABLE_ACCOUNT_IDS = "(SELECT " + AccountsColumns._ID
                + " FROM " + Tables.ACCOUNTS + " WHERE ("
                + AccountsColumns.ACCOUNT_NAME + " IS NULL AND "
                + AccountsColumns.ACCOUNT_TYPE + " IS NULL AND "
        ...
        public static final String RAW_SQL =
                ...
                " WHERE " + Data.RAW_CONTACT_ID + "=?" +
                        " AND (" + Data.DATA1 + " NOT NULL OR " +
                                ...
                                */
                                Organization.TITLE + " NOT NULL) " +
                                " ORDER BY " + DataColumns.MIMETYPE_ID +
                                ", " + Data.DATA1;
                                /**
                                * @}
                                */
    ...
    protected ContactsDatabaseHelper(
        ...
        /**
         * SPRD: Bug819475 Optimize contact cold start performance
         * Original Android code:
...
//        ");");
    ...
    private void createContactsViews(SQLiteDatabase db) {
        ...
        String contactNameColumns =
                 ...
                 */
                + AccountsColumns.CONCRETE_ACCOUNT_NAME
                + " AS " + Contacts.DISPLAY_ACCOUNT_NAME + ", "
                + AccountsColumns.CONCRETE_ACCOUNT_TYPE
                + " AS " + Contacts.DISPLAY_ACCOUNT_TYPE + ", "
                /* @} */
               /**
                ...
                * @{
                */
                + "name_raw_contact." + RawContacts.SYNC1
                + " AS " + RawContacts.SYNC1 + ", "
                + "name_raw_contact." + RawContacts.SYNC2
                + " AS " + RawContacts.SYNC2 + ", "
                /**
                 * @}
                 */
        ...
        String contactsSelect = "SELECT "
                ...
                + "name_raw_contact." + RawContacts.RAW_CONTACT_IS_READ_ONLY
                + " AS " + " is_read_only, "
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        //SPRD: add for bug693208, AndroidO porting for contacts to add Phone and SIM account
        if (isUpgradeRequired(oldVersion, newVersion, 1203)) {
            upgradeViewsAndTriggers = true;
            oldVersion = 1203;
        }
        /**
         * SPRD:Bug 693219 Adding a default people  androidO_porting.
         * @{
         */
        if (isUpgradeRequired(oldVersion, newVersion, 1205)) {
            upgradeViewsAndTriggers = true;
            oldVersion = 1205;
        }
        /**
         ...
         * @{
         */
        if (isUpgradeRequired(oldVersion, newVersion, 1206)) {
            upgradeViewsAndTriggers = true;
            oldVersion = 1206;
        }
        /**
        ...
        */
         ...
         * @{
         */
        if (isUpgradeRequired(oldVersion, newVersion, 1207)) {
            upgradeViewsAndTriggers = true;
            oldVersion = 1207;
        }
        /**
        ...
        */
        ...
        if (isUpgradeRequired(oldVersion, newVersion, 1400)) {
            ContactsProvider2.deleteDefaultContacts(db);
     ...
     * @{
     */
    public long getMimeTypeIdForWebsite() {
        return lookupMimeTypeId(getWritableDatabase(), Website.CONTENT_ITEM_TYPE);
    }
    /**
     * @}
     */
    ...
    public String getGroupTitle(long groupId) {
        final SQLiteStatement query = getReadableDatabase().compileStatement(
                "SELECT " + Groups.TITLE +
                        " FROM " + Tables.GROUPS +
        ...
        String ret = null;
        try {
            ret = query.simpleQueryForString();
        } catch (SQLiteDoneException e) {
        }
    ...
    public AccountWithDataSet getAccount(Long accountId) {
        if (accountId == null) {
            return null;
        }
        Cursor cursor = getReadableDatabase().query(Tables.ACCOUNTS,
                new String[] {
                        AccountsColumns.ACCOUNT_NAME,
                        AccountsColumns.ACCOUNT_TYPE,
                        AccountsColumns.DATA_SET,
                },
                AccountsColumns._ID + " = " + accountId,
                null, null, null, null);
        try {
            if (cursor.moveToFirst()) {
                return new AccountWithDataSet(cursor.getString(0), cursor.getString(1),
                        cursor.getString(2));
            } else {
                return null;
            }
    ...
    private SQLiteStatement mRawContactIdentityUpdate;

    public void updateRawContactIdentity(SQLiteDatabase db, long rawContactId) {
        mSelectionArgs1[0] = String.valueOf(rawContactId);
        Cursor c = db.rawQuery(RawContactNameQuery.RAW_SQL, mSelectionArgs1);
        ...
        try {
            if (c.moveToFirst()) {
                do {
                    int mimeType = c.getInt(RawContactNameQuery.MIMETYPE);
                    if ((mimeType == getMimeTypeIdForStructuredName() ||
                            mimeType == getMimeTypeIdForEmail() ||
                            mimeType == getMimeTypeIdForPhone())
                            && (c.getBlob(RawContactNameQuery.DATA1) != null))
                    {
                        if (mimeType == getMimeTypeIdForPhone()) {
                            String number= c.getString(RawContactNameQuery.DATA1);
                            String formattedNumber = formattNumber(number);
                            try {
                                digester.update(formattedNumber.getBytes("UTF-8"));
                            } catch (java.io.UnsupportedEncodingException e) {
                                e.printStackTrace();
                                return;
                            }
                        } else {
                            digester.update(c.getBlob(RawContactNameQuery.DATA1));
                        }
                    }
                } while (c.moveToNext());
            }
        ...
        if (mRawContactIdentityUpdate == null) {
            mRawContactIdentityUpdate = db.compileStatement(
                    "UPDATE " + Tables.RAW_CONTACTS +
                            " SET " +
                            RawContacts.SYNC4 + "=?" +
                            " WHERE " + RawContacts._ID + "=?");
        }
        mRawContactIdentityUpdate.bindString(1,
                Base64.encodeToString(digester.digest(), Base64.DEFAULT));
        mRawContactIdentityUpdate.bindLong(2, rawContactId);
        mRawContactIdentityUpdate.execute();
    }

    private String formattNumber(String actualNumber) {
        final int length = actualNumber.length();
        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < length; i++) {
            char ch = actualNumber.charAt(i);
            if (('0' <= ch && ch <= '9') || (i == 0 && ch == '+')) {
                builder.append(ch);
            }
        }
