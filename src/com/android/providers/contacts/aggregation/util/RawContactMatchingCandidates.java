...
public class RawContactMatchingCandidates {
    ...
    public void remove(long id) {
        if(mBestMatches != null) {
            for(MatchScore m : mBestMatches) {
                if(m.getRawContactId() == id) {
                    mBestMatches.remove(m);
                    break;
                }
            }
        }
        if (mRawContactIds != null) {
            mRawContactIds.remove(id);
        }
        if (mRawContactToAccount != null) {
            mRawContactToAccount.remove(id);
        }
        if (mRawContactToContact != null) {
            mRawContactToContact.remove(id);
        }
    }
    /**
     * @}
