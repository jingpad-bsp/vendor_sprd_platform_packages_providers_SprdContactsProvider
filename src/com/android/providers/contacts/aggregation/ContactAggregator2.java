...
public class ContactAggregator2 extends AbstractContactAggregator {
     ...
     */
     synchronized void aggregateContact(TransactionContext txContext, SQLiteDatabase db,
        ...
        if (aggregationMode == RawContacts.AGGREGATION_MODE_DEFAULT) {
         ...
         /**
          * SPRD: Bug708552 The sim contact should not be merged into phone contact.
          * @{
          */
         List<Long> aIds = getSimAccountId(db);
         Set<Long> ids = matchingCandidates.getRawContactIdSet();
         ...
         for (Long id : ids) {
             if(isFirst) {
                 ...
                 where.append(" OR _id=" + id);
             }
         }
         Cursor c = db.query(Tables.RAW_CONTACTS, new String[]{RawContactsColumns.ACCOUNT_ID,
                 BaseColumns._ID}, where.toString(), null, null, null, null);
         try {
             while (c.moveToNext()) {
                 long aId = c.getLong(0);
                 if (aIds.contains(aId)) {
                     matchingCandidates.remove(c.getLong(1));
                 }
             }
         } finally {
             c.close();
     ...
     * @{
     */
    private List<Long> getSimAccountId(SQLiteDatabase db) {
        ArrayList<Long> simAccountIds = new ArrayList<Long>();
        String where = AccountsColumns.ACCOUNT_TYPE + "='sprd.com.android.account.sim'" + " OR " +
                AccountsColumns.ACCOUNT_TYPE + "='sprd.com.android.account.usim'";

        Cursor c = db.query(Tables.ACCOUNTS, new String[]{BaseColumns._ID},
                where, null, null, null, null);
        try {
            while (c.moveToNext()) {
                long aId = c.getLong(0);
                simAccountIds.add(aId);
            }
        } finally {
            c.close();
        }
        return simAccountIds;
    }
    /**
     * @}
     */
                ...
                if (!firstLetters.contains(firstLetter)) {
                    ...
                    final String selection = "(" + NameLookupColumns.NORMALIZED_NAME + " GLOB '"
                            ...
                            + "(" + NameLookupColumns.NAME_TYPE + " IN("
                                    ...
                                    + "raw_contact_is_read_only=0 AND "
                                    /* @} */
    ...
    protected interface NameLookupMatchQuery {
        ...
        String SELECTION = "nameA." + NameLookupColumns.RAW_CONTACT_ID + "=?"
               /* Bug 693219 Adding a default people  androidO_porting @{ */
                + " AND raw_contact_is_read_only=0"
                /* @} */
                + " AND " + RawContacts.CONTACT_ID + " IN " + Tables.DEFAULT_DIRECTORY;
