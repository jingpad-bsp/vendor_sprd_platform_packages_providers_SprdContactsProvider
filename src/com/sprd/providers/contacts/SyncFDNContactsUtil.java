...
public class SyncFDNContactsUtil {
    ...
    //UNISOC: add for bug621877/1164026, fdn feature bugfix
    public boolean purgeFDNContacts(Context context, String phoneId) {
        Log.d(TAG, " ---> purgeFdnContact : " + phoneId);
        String where = RawContacts.SYNC2 + " = 'fdn" + phoneId + "'";
        try {
            if (mImportTask != null) {
                mImportTask.cancel(true);
                mCancelPhoneId = Integer.parseInt(phoneId);
            }
            context.getContentResolver().delete(RawContacts.CONTENT_URI, where, null);
        } catch (Exception e) {
